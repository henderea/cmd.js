...
class Command extends EventEmitter {
    ...
    def(...defs) {
        argsert('<string...>', [...defs], ['defs'], arguments.length);
        if(!_.isArray(defs)) {
            defs = [defs];
        }
    ...
    desc(desc) {
        argsert('<string>', [desc], ['desc'], arguments.length);
    ...
    action(action) {
        argsert('<function>', [action], ['action'], arguments.length);
        this.on('command', action);
        return this;
    }

    usage(usage) {
        argsert('<string>', [usage], ['usage'], arguments.length);
        this._usage = usage;
        return this;
    }

    command(init) {
        argsert('<function|Command|string>', [init], ['init'], arguments.length);
        let cmd = null;
    ...
    commandDir(dirName) {
        argsert('<string>', [dirName], ['dirName'], arguments.length);
